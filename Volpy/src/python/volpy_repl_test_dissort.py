...
MagicSortNumPerCore = (10**5)*3 # Max memory for grpc message
...
def quick_sort_distributed(collection):
    ...
    print(len(collection), MagicSortNumPerCore)
    if len(collection) <= MagicSortNumPerCore:  # magic number (Lowered for unittest)
        return sorted(collection)
    else:
        lesser, pivot, greater = partition(collection)
        lesser = quick_sort_distributed.remote(lesser).get()
        greater = quick_sort_distributed.remote(greater).get()
        return lesser + [pivot] + greater
...
async def test_repl(i=0):
    ...
    Volpy.registerRemote(quick_sort_distributed)
    t1 = time.time()
    sort = quick_sort_distributed.remote(unsorted).get()
    t2 = time.time()
