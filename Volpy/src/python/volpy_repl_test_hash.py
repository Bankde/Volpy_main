...
async def hashSearchFromWordSubs_dist(s, cur, i, substitute, h, spawn):
    if i >= len(s):
        if hashlib.sha512(cur.encode('utf-8')).hexdigest() == h:
            ...
            return None
    volpy_task = set() # Storing runTask
    for c in substitute[s[i]]:
        if spawn:
            try:
                volpy_task.add(await hashSearchFromWordSubs_dist.remote(s, cur+c, i+1, substitute, h, spawn))
            except:
                spawn = False
                r = await hashSearchFromWordSubs_dist(s, cur+c, i+1, substitute, h, False)
                if r != None:
    ...
    pending = set() # Storing future of Volpy.get()
    for t in volpy_task:
        pending.add(t.get())
    # Any Volpy.get() task is done then get that one first
    while len(pending) > 0:
