...
if pickle.HIGHEST_PROTOCOL >= 5 and not PYPY:
    ...
    def dump(obj, file, protocol=None, buffer_callback=None):
        raise NotImplementedError("No write to file support")
    ...
    def dumps(obj, protocol=None):
        ...
        """
        with io.BytesIO() as file:
            cp = CodePickler(file, protocol=protocol)
            cp.dump(obj)
            if CONFIG_GET_IMPORT:
                return file.getvalue(), cp.getImportList()
            else:
...
def _function_getstate(func):
    ...
    # This theoretically means that exec(code) CAN YIELD INCORRECT result and
...
class CodePickler(Pickler):
    ...
    if pickle.HIGHEST_PROTOCOL >= 5:
        ...
        raise NotImplemented("For simplicity, only support pickle protocol 5")
