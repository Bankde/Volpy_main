...
def _make_cwd_env():
    ...
    codepickle_repo_folder = op.normpath(
        op.join(op.dirname(__file__), '..'))
    env = os.environ.copy()
    pythonpath = "{src}{sep}tests{pathsep}{src}".format(
        src=codepickle_repo_folder, sep=os.sep, pathsep=os.pathsep)
    env['PYTHONPATH'] = pythonpath
    return codepickle_repo_folder, env
...
def call_func(payload, protocol):
    """Remote function call that uses codepickle to transport everthing"""
...
def assert_run_python_script(source_code, timeout=TIMEOUT):
    ...
    """
    fd, source_file = tempfile.mkstemp(suffix='_src_test_codepickle.py')
    os.close(fd)
    try:
