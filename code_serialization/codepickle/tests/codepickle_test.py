...
import codepickle
from codepickle.compat import pickle
from codepickle import register_pickle_by_value
from codepickle import unregister_pickle_by_value
from codepickle import list_registry_pickle_by_value
from codepickle.codepickle import _should_pickle_by_reference
from codepickle.codepickle import _make_empty_cell, cell_set
from codepickle.codepickle import _extract_class_dict, _whichmodule
from codepickle.codepickle import _lookup_module_and_qualname
...
from _codepickle_testpkg import relative_imports_factory
...
def pickle_depickle(obj, protocol=codepickle.DEFAULT_PROTOCOL):
    """Helper function to test whether object pickled with codepickle can be
    depickled with pickle
    """
    return pickle.loads(codepickle.dumps(obj, protocol=protocol))
...
class CodePickleTest(unittest.TestCase):

    protocol = codepickle.DEFAULT_PROTOCOL

    def setUp(self):
        self.tmpdir = tempfile.mkdtemp(prefix="tmp_codepickle_test_")
    ...
    def test_reducer_override_reference_cycle(self):
        ...
        # codepickle and Python 3.8.0/1, hence the skipif directive.
        class MyClass:
            pass
    ...
    def test_pickling_file_handles(self):
        out1 = sys.stderr
        out2 = pickle.loads(codepickle.dumps(out1, protocol=self.protocol))
    ...
    def test_func_globals(self):
        ...
        self.assertRaises(Exception, lambda: codepickle.dumps(
    ...
    def test_loads_namespace(self):
        obj = 1, 2, 3, 4
        returned_obj = codepickle.loads(codepickle.dumps(
    ...
    def test_load_namespace(self):
        obj = 1, 2, 3, 4
        bio = io.BytesIO()
        codepickle.dump(obj, bio)
        bio.seek(0)
        returned_obj = codepickle.load(bio)
        self.assertEqual(obj, returned_obj)
    ...
    def test_module_locals_behavior(self):
        ...
        try:
            ...
            with open(pickled_func_path, 'wb') as f:
                codepickle.dump(g, f, protocol=self.protocol)
    ...
    def test_dynamic_module_with_unpicklable_builtin(self):
        # Reproducer of https://github.com/cloudpipe/codepickle/issues/316
        ...
        # the '__builtins__' key. In such cases, codepickle used to fail
        ...
        with pytest.raises(ValueError):
            codepickle.dumps(unpicklable_obj)
    ...
    def test_load_dynamic_module_in_grandchild_process(self):
        ...
        child_process_script = '''
            ...
            with open('{child_process_module_file}', 'wb') as f:
                codepickle.dump(mod, f, protocol={protocol})
        ...
        try:
            with open(parent_process_module_file, 'wb') as fid:
                codepickle.dump(mod, fid, protocol=self.protocol)
    ...
    def test_correct_globals_import(self):
        ...
        b = codepickle.dumps(my_small_function, protocol=self.protocol)
    ...
    def test_module_importability(self):
        ...
        # their parent modules are considered importable by codepickle.
        # See the mod_with_dynamic_submodule documentation for more
        # details of this use case.
        import _codepickle_testpkg.mod.dynamic_submodule as m
        ...
        # attribute lookup.
        from _codepickle_testpkg.mod import dynamic_submodule_two as m2
        # Note: import _codepickle_testpkg.mod.dynamic_submodule_two as m2
        ...
        with pytest.raises(ImportError):
            import _codepickle_testpkg.mod.submodule_three  # noqa
        from _codepickle_testpkg.mod import submodule_three as m3
        ...
        import _codepickle_testpkg.mod.dynamic_submodule.dynamic_subsubmodule as sm  # noqa
    ...
    def test_builtin_function(self):
        # Note that builtin_function_or_method are special-cased by codepickle
        # only in python2.
    ...
    def test_tornado_coroutine(self):
        ...
        data = codepickle.dumps([g, g], protocol=self.protocol)
        f = g = None
    ...
    def test_extended_arg(self):
        ...
        f = d['f']
        res = f()
        data = codepickle.dumps([f, f], protocol=self.protocol)
        d = f = None
    ...
    def test_submodule(self):
        ...
        s = codepickle.dumps(example, protocol=self.protocol)
    ...
    def check_logger(self, name):
        ...
        dumped = codepickle.dumps(logger)

        code = """if 1:
            import base64, codepickle, logging

            logging.basicConfig(level=logging.INFO)
            logger = codepickle.loads(base64.b32decode(b'{}'))
            logger.info('hello')
    ...
    def test_logger(self):
        ...
        self.check_logger('codepickle.dummy_test_logger')
    ...
    def test_abc_cache_not_pickled(self):
        # codepickle issue #302: make sure that codepickle does not pickle
    ...
    def test_abstracts(self):
        ...
        # See https://github.com/cloudpipe/codepickle/issues/367
    ...
    def test_non_module_object_passing_whichmodule_test(self):
        # https://github.com/cloudpipe/codepickle/pull/326: codepickle should
    ...
    def test_dynamic_pytest_module(self):
        # Test case for pull request https://github.com/cloudpipe/codepickle/pull/116
        import py
        ...
        # serializable.
        from codepickle import CodePickler
        from codepickle import codepickle_fast as cp_fast
        CodePickler.dispatch_table[type(py.builtin)] = cp_fast._module_reduce

        g = codepickle.loads(codepickle.dumps(f, protocol=self.protocol))
    ...
    def test_interactively_defined_global_variable(self):
        ...
        code_template = """\
        from testutils import subprocess_pickle_echo
        from codepickle import dumps, loads
        ...
        # thanks to codepickle isolation, depickling and calling f0 and f1
    ...
    def test_closure_interacting_with_a_global_variable(self):
        ...
        try:
            ...
            # pickle f1 another time, but in a new pickle string
            pickled_f1 = codepickle.dumps(f1, protocol=self.protocol)
    ...
    def test_interactive_dynamic_type_and_stored_remote_instances(self):
        ...
        with subprocess_worker(protocol={protocol}) as w:
            ...
            def store(x):
                storage = getattr(codepickle, "_test_storage", None)
                if storage is None:
                    storage = codepickle._test_storage = dict()
                obj_id = uuid.uuid4().hex
                storage[obj_id] = x
                return obj_id

            def lookup(obj_id):
                return codepickle._test_storage[obj_id]
    ...
    def test_pickle_reraise(self):
        for exc_type in [Exception, ValueError, TypeError, RuntimeError]:
            ...
            with pytest.raises((exc_type, pickle.PicklingError)):
                codepickle.dumps(obj, protocol=self.protocol)
    ...
    def test_instance_with_slots(self):
        for slots in [["registered_attribute"], "registered_attribute"]:
            ...
            s = codepickle.dumps(obj, protocol=self.protocol)
            del SubclassWithSlots
            depickled_obj = codepickle.loads(s)
    ...
    def test_relative_import_inside_function(self):
        ...
        # broken. This was a bug in codepickle versions <= 0.5.3 and was
    ...
    def test_interactively_defined_func_with_keyword_only_argument(self):
        # fixes https://github.com/cloudpipe/codepickle/issues/263
        def f(a, *, b=1):
            return a + b
    ...
    def test_interactively_defined_func_with_positional_only_argument(self):
        # Fixes https://github.com/cloudpipe/codepickle/issues/266
        ...
        code = """
        import pytest
        from codepickle import loads, dumps
    ...
    def test___reduce___returns_string(self):
        ...
        from _codepickle_testpkg import some_singleton
    ...
    def test_codepickle_extract_nested_globals(self):
        ...
        globals_ = set(codepickle.codepickle._extract_code_globals(
    ...
    def test_out_of_band_buffers(self):
        ...
        buffers = []
        pickle_bytes = codepickle.dumps(data_instance, protocol=self.protocol,
    ...
    def test_pickle_importable_typevar(self):
        from _codepickle_testpkg import T
    ...
    def test_literal_misdetection(self):
        # see https://github.com/cloudpipe/codepickle/issues/403
        class MyClass:
            @property
            def __values__(self):
                return ()
    ...
    def test_pickle_constructs_from_module_registered_for_pickling_by_value(self):  # noqa
        ...
        try:
            ...
            # - we start from the /path/to/codepickle/tests directory, where a
            ...
            #   (un)register_pickle_by_value(module) api that codepickle
            #   exposes.
            _mock_interactive_session_cwd = os.path.dirname(__file__)

            # First, remove sys.path entries that could point to
            # /path/to/codepickle/tests and be in inherited by the worker
    ...
    def test_pickle_various_versions_of_the_same_function_with_different_pickling_method(  # noqa
        ...
        # worker.
        import _codepickle_testpkg
        from _codepickle_testpkg import package_function_with_global as f
        _original_global = _codepickle_testpkg.global_variable

        def _create_registry():
            _main = __import__("sys").modules["__main__"]
            _main._codepickle_registry = {}
            # global _codepickle_registry

        def _add_to_registry(v, k):
            _main = __import__("sys").modules["__main__"]
            _main._codepickle_registry[k] = v

        def _call_from_registry(k):
            _main = __import__("sys").modules["__main__"]
            return _main._codepickle_registry[k]()
        ...
        try:
            with subprocess_worker(protocol=self.protocol) as w:
                ...
                register_pickle_by_value(_codepickle_testpkg)
                _codepickle_testpkg.global_variable = "modified global"
            ...
            _codepickle_testpkg.global_variable = _original_global

            if "_codepickle_testpkg" in list_registry_pickle_by_value():
                unregister_pickle_by_value(_codepickle_testpkg)
...
def test_lookup_module_and_qualname_importable_typevar():
    import _codepickle_testpkg
    T = _codepickle_testpkg.T
    module_and_name = _lookup_module_and_qualname(T, name=T.__name__)
    assert module_and_name is not None
    module, name = module_and_name
    assert module is _codepickle_testpkg
    assert name == 'T'
...
def test_register_pickle_by_value():
    import _codepickle_testpkg as pkg
    import _codepickle_testpkg.mod as mod
...
def test_module_level_pickler():
    # #366: codepickle should expose its pickle.Pickler subclass as
    # codepickle.Pickler
    assert hasattr(codepickle, "Pickler")
    assert codepickle.Pickler is codepickle.CodePickler
