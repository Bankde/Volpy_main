...
class CodePickleFileTests(unittest.TestCase):
    ...
    def test_empty_file(self):
        ...
        with open(self.tmpfilepath, 'r') as f:
            self.assertEqual('', pickle.loads(codepickle.dumps(f)).read())
    ...
    def test_r_mode(self):
        ...
        # Open for reading
        with open(self.tmpfilepath, 'r') as f:
            new_f = pickle.loads(codepickle.dumps(f))
    ...
    def test_seek(self):
        ...
        with open(self.tmpfilepath, 'w+') as f:
            f.write(self.teststring)
            f.seek(4)
            unpickled = pickle.loads(codepickle.dumps(f))
    ...
    def test_pickling_special_file_handles(self):
        ...
        for out in sys.stdout, sys.stderr:  # Regression test for SPARK-3415
            self.assertEqual(out, pickle.loads(codepickle.dumps(out)))
        self.assertRaises(pickle.PicklingError,
                          lambda: codepickle.dumps(sys.stdin))
